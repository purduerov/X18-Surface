...
class HatTempSubscriber(Node):
    def __init__(self):
        super().__init__("hat_temp_subscriber")

        # Dictionary to map topic names to their corresponding callback functions
        self.topics = {
            "hat_temp": self.rov_hat_temp_callback,
        }

        # Subscribe to each topic in the topics dictionary
        self.create_subscription(Float32, "hat_temp", self.rov_hat_temp_callback, 10)

    def rov_hat_temp_callback(self, msg):
        msg_dict = rosmsg_to_dict(msg)
        #self.get_logger().info(f'test')
        msg_json = json.dumps(msg_dict)
        #self.get_logger().info(f'Received from hat_temp topic: "{msg}"')
        if sio.connected:
            sio.emit('hat_temp', msg_json)
            self.get_logger().info(f'Emitted hat_temp data to SocketIO: {msg_json}')
        else:
...
def main():
    rclpy.init()
    hat_temp_subscriber_node = HatTempSubscriber()

    port = str(os.getenv("FLASK_PORT", 5013))

    try:
        sio.connect("http://127.0.0.1:" + port)
        hat_temp_subscriber_node.get_logger().info(f"Connected to SocketIO server at port {port}")
    except Exception as e:
        hat_temp_subscriber_node.get_logger().error(f"Failed to connect to SocketIO server: {e}")

    rclpy.spin(hat_temp_subscriber_node)
    hat_temp_subscriber_node.destroy_node()
    rclpy.shutdown()
